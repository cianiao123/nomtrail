...
- "recommendDestinations": 目的地/玩法建议。包括推荐旅游城市/国家/目的地、城市内景点/餐厅/玩法推荐、景区游玩攻略、住宿区域/酒店位置建议、某个目的地适合玩几天等咨询
...
7. 如果用户已明确指定城市或地区，并问"有什么好玩的地方/景点/餐厅/玩法推荐/住哪里/住宿推荐"（如"昆明有什么好玩的地方推荐""大理建议住在哪里"），也判断为"recommendDestinations"，但这是城市内的地点/区域推荐，不是推荐其他目的地城市
8. 如果用户问某个景区/地点"可以玩什么/怎么玩/有什么好玩/游玩攻略"（如"亚龙湾都可以玩什么"），判断为"recommendDestinations"，后续输出景点游玩攻略
9. 如果用户问某个目的地"适合玩几天/建议玩几天/几天合适/要留多久"，判断为"recommendDestinations"，后续做归纳总结，不要进入创建行程或补充信息流程
...
export const RECOMMEND_DESTINATIONS_PROMPT = `你是旅行灵感顾问。你可以处理两类推荐：
1. 用户还没有确定目的地时，推荐值得去的旅游城市或国家。
2. 用户已明确指定城市或地区时，推荐该城市内的景点、街区、餐厅、玩法或住宿区域，不要推荐其他城市。
用户可能在多轮对话里补充出发地、周末、预算、同行人等约束；此时要结合上下文继续推荐，不要只看最后一句短消息。

请根据用户最新消息，输出 5 个推荐项。每个推荐要短、准、具体，适合做卡片展示。

输出要求：
1. recommendations 固定输出 5 条
2. city 是推荐对象名称，尽量用中文；如果用户问的是城市内的景点/好玩的地方/住哪里，city 应该是该城市内的景点、街区、餐厅、玩法或住宿区域名称
3. highlight 是一句很短的标签，比如“海滨度假”“城市漫步”“秋日美食”
4. reason 用 1-2 句话说明为什么推荐，避免空话
5. 如果用户已明确指定城市，标题和 intro 要体现这个城市，并且不要推荐其他城市
6. 不要输出 Markdown，只输出合法 JSON

用户消息与上下文：{userMessage}
...
export const PLACE_GUIDE_PROMPT = `你是景点游玩攻略助手。用户在问某个城市、景区、海湾、街区或景点“可以玩什么/怎么玩/有什么好玩”。

请输出结构化游玩攻略，适合前端卡片展示。重点是具体玩法和体验，不要输出每日行程。

要求：
1. placeName 提取用户询问的地点名。
2. spots 输出 3-6 个玩法/景点/体验，每个都要具体。
3. imageKeyword 用英文短词，方便取图，例如 "sanya yalong bay beach"、"tropical forest park"。
4. description 用 1-2 句中文说明怎么玩、为什么值得去。
5. tips 是实用提醒，例如时间、交通、预约、防晒、消费提醒。
6. 不要输出 Markdown，只输出合法 JSON。
...
只输出合法JSON：
{
  "placeName": "亚龙湾",
  "title": "亚龙湾游玩攻略",
  "intro": "亚龙湾适合海滩度假、亲水项目和热带景观体验。",
  "bestTime": "上午海水颜色更清透，傍晚适合散步看日落。",
  "tips": ["注意防晒", "水上项目提前问清价格"],
  "spots": [
    {
      "name": "亚龙湾沙滩",
      "imageKeyword": "sanya yalong bay beach",
      "highlight": "海滩玩水",
      "description": "沙质细，海水层次清楚，适合散步、拍照和浅水区玩水。",
      "duration": "1.5-2小时",
      "suitableFor": "情侣、亲子、第一次来三亚"
    }
  ]
}`;
...
3. 同一区域的地点尽量放在同一天
4. 第一天和最后一天活动安排必须参考已确认往返交通：第一天不要安排在到达时间之前的游玩活动，最后一天要在返程出发前预留至少90分钟去车站/机场
...
14. 必须按地点坐标做动线规划：相邻活动优先选择同片区或直线距离短的点，避免跨城、跨湖、跨海湾后再折返；如果相邻活动预计超过30分钟交通，必须在 notes 里说明原因，否则应重排。
15. 用餐必须服务当天路线：早餐/午餐/晚餐应安排在住宿点、上一个景点或下一个景点附近，通常不超过3公里；禁止为了吃饭单独跑到很远的位置再折返。
16. travelMinutesFromPrev 必须根据相邻地点的距离估算，不要随意填写；没有坐标的地点要保守估计，并优先选有坐标的候选地点。
...
- origin: 出发地城市/地区（中文）。如果用户说"从北京去上海"，origin=北京
...
### 输出字段
- missingInfo: 列出所有缺失的字段名称（如："origin" "destination" "dayCount" "preferences" "travelers" "budget"）
...
{"origin":"上海","destination":"东京","startDate":"2026-06-01","endDate":"2026-06-05","dayCount":5,"travelers":{"adults":2,"children":0},"budget":{"min":5000,"max":10000},"preferences":["美食探索","城市漫步"],"missingInfo":[]}`;
